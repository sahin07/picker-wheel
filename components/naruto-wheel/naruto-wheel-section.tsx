...
import { getAllNarutoEntries } from "@/data/naruto-characters"
...
import type { ActionMode, NarutoEntry, SpinResult } from "@/types/naruto-types"
...
export default function NarutoWheelSection({
  ...
  const [winner, setWinner] = useState<NarutoEntry | null>(null)
  ...
  const pendingWinner = useRef<NarutoEntry | null>(null)
  ...
  const allEntries = useMemo(
    () => [...getAllNarutoEntries(), ...(Array.isArray(data?.customCharacters) ? data.customCharacters : [])],
    [data?.customCharacters],
  )
  const activeEntries = useMemo(() => {
    const selected = new Set(Array.isArray(data?.selectedCharacters) ? data.selectedCharacters : [])
    const ordered = Array.isArray(data?.characterOrder) ? data.characterOrder : []
    const byId = new Map(allEntries.map((item) => [item.id, item]))
    const first = ordered.map((id) => byId.get(id)).filter(Boolean) as NarutoEntry[]
  ...
  const modalResults = useMemo(
    () =>
      (Array.isArray(data?.recentResults) ? data.recentResults : []).slice(0, 10).map((result) => ({
        id: result.id,
        name: result.name,
        emoji: result.emoji,
  ...
  const updateData = useCallback((partial: Partial<NarutoWheelData>) => {
    if (!wheel || !data) return
    const latest = useWheelManagerStore.getState().getCurrentWheel()?.data as NarutoWheelData | undefined
    useWheelManagerStore.getState().updateWheelData("naruto-wheel", wheel.id, { ...(latest || data), ...partial })
  }, [data, wheel])
  ...
  useEffect(() => {
    ...
    window.addEventListener("open-naruto-results", open)
    return () => window.removeEventListener("open-naruto-results", open)
  }, [])
  ...
  const handleSpinComplete = useCallback(() => {
    ...
    updateData({
      ...
      spinHistory: [{
        id: `naruto-spin-${Date.now()}`, timestamp: new Date(), result: result.name,
  ...
  return <div className={isFullscreen
    ...
    {confettiBurst > 0 && typeof document !== "undefined" && createPortal(
      ...
      <div ref={wheelBoxRef} className={`relative w-full max-w-[680px] overflow-visible ${!isSpinning && canvasItems.length ? "cursor-pointer" : ""}`}>
        <button type="button" className="block w-full" onClick={spin}
          disabled={isSpinning || !canvasItems.length} aria-label="Spin Naruto character wheel">
        ...
        {isSpinning && (
          <div className="absolute right-2 top-2 z-20 animate-pulse rounded-full bg-orange-600 px-2 py-1 text-xs font-semibold text-white sm:right-4 sm:top-4 sm:px-3 sm:text-sm">
    ...
    {winner && !isSpinning && (
      <div className="w-full max-w-md rounded-xl border-2 border-orange-300 bg-orange-50 p-5 text-center shadow-sm">
        ...
        <p className="text-4xl">{winner.emoji}</p>
        <p className="text-sm font-semibold uppercase tracking-wide text-orange-700">Selected</p>
        <p className="mt-1 text-2xl font-bold text-orange-950">{winner.name}</p>
        {winner.blurb && <p className="mt-2 text-sm text-slate-600">{winner.blurb}</p>}
        <p className="mt-2 text-xs text-slate-500">
          {[winner.village, winner.clan, winner.rank].filter(Boolean).join(" · ")}
        </p>
        <div className="mt-3 flex flex-wrap justify-center gap-2">
          <Button type="button" size="sm" variant="outline" onClick={() => {
            const keep = Array.isArray(data?.keepWinnerIds) ? data.keepWinnerIds : []
            if (!keep.includes(winner.id)) updateData({ keepWinnerIds: [...keep, winner.id] })
          }}>Keep selected</Button>
          <Button type="button" size="sm" variant="outline" onClick={() => {
            const text = `I spun ${winner.name} on the Naruto Wheel!`
          ...
          <Button type="button" size="sm" variant="outline" onClick={spin}>Re-spin</Button>
        </div>
        {!!data?.keepWinnerIds?.length && (
          <p className="mt-3 text-xs text-orange-800">Kept: {(data.keepWinnerIds || []).length} character(s)</p>
        )}
      </div>
    )}

    <Button type="button" onClick={spin} disabled={isSpinning || !canvasItems.length}
      className="w-full max-w-sm bg-gradient-to-r from-orange-600 to-red-600 px-6 py-3 text-base font-semibold text-white shadow-lg sm:px-12 sm:text-lg">
      {isSpinning
        ...
        : `SPIN Naruto WHEEL (${activeEntries.length})`}
    </Button>
    ...
    {onActionModeChange && (
      <div className="mt-2 w-full max-w-md rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 to-red-50 p-3 sm:mt-4 sm:p-4">
        <label className="mb-2 flex items-center text-sm font-semibold text-orange-800 sm:mb-3">
          <span className="mr-2 h-2 w-2 rounded-full bg-orange-500" />Game Mode
        </label>
        <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
          {([
            ...
            <label key={mode.value}
              ...
              <input type="radio" name="narutoActionMode" value={mode.value} className="sr-only"
                checked={actionMode === mode.value}
                onChange={() => onActionModeChange(mode.value)} />
              <span className="text-sm">{mode.icon}</span>
              <span className={`text-[10px] font-semibold sm:text-xs ${actionMode === mode.value ? "text-white" : "text-orange-700"}`}>
                {mode.label}
              </span>
            </label>
          ))}
        </div>
        <div className="mt-2 rounded-lg bg-orange-50 p-2 text-center text-[11px] text-orange-600 sm:mt-3 sm:text-xs">
    ...
    <div className="flex justify-center">
      <div className="rounded-lg bg-gradient-to-r from-orange-600 to-red-600 px-4 py-2 text-white shadow-lg">
    ...
    {!canvasItems.length && <p className="text-sm text-slate-500">Enable at least one Naruto entry to spin.</p>}

    {isGameActive && currentGameModeName && (
      <div className="w-full max-w-md rounded-lg border-2 border-orange-300 bg-gradient-to-r from-orange-100 to-red-100 p-2.5 sm:p-3">
        <p className="text-xs font-semibold text-orange-800 sm:text-sm">Playing: {currentGameModeName}</p>
      </div>
    )}
