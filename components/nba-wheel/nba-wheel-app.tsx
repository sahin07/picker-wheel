...
import NBAInputPanel from "@/components/nba-input-panel"
...
import { type ActionMode, type NBATeam } from "@/data/nba-teams"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { PanelRightOpen } from "lucide-react"
import { NbaWheelPopularTemplates } from "@/components/nba-wheel/nba-wheel-popular-templates"
import { NBA_WHEEL_SHORT_TITLE } from "@/lib/nba-wheel-seo"
import type { NbaWheelDeepLink } from "@/lib/nba-wheel-spokes"
import {
  applyNbaWheelUseCase,
  getNbaWheelUseCase,
  nbaWheelUseCaseFromTemplate,
  type NbaWheelUseCaseId,
} from "@/lib/nba-wheel-use-cases"

type NbaWheelAppProps = {
  seoIntro?: ReactNode
...
function NbaWheelAppInner({
  ...
  const [activeUseCaseId, setActiveUseCaseId] = useState<NbaWheelUseCaseId | null>(
    ...
    const initializeApp = async () => {
      try {
        ...
        if (typeof window !== "undefined") {
          ...
          if (!wheel) {
            const id = `nba-wheel-${Date.now()}`;
            const now = new Date().toISOString();
            createNewWheel("nba-wheel", "NBA Picker Wheel", id, now, now);
          }
        }
        ...
        console.error("Error initializing NBA wheel:", error)
      }
    }
    initializeApp()
  ...
  const applyUseCasePreset = useCallback((id: NbaWheelUseCaseId) => {
    if (!applyNbaWheelUseCase(id)) return
  ...
  const handleAddManualTeam = useCallback((teamName: string) => {
    ...
    if (currentWheel) {
      ...
      const existingTeam = currentData.selectedTeams.find((team: NBATeam) => 
      ...
      >
        ...
        <main className="w-full min-w-0 overflow-x-hidden px-3 py-4 sm:px-6 sm:py-8 lg:px-8">
          <div className="mb-4 text-center">
            <ToolPageTitle
              title={shortTitle ?? NBA_WHEEL_SHORT_TITLE}
              toolType="nba-wheel"
            />
          ...
          <NbaWheelPopularTemplates />

          {activeUseCaseId && (
            <div className="mb-4 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              <Badge className="bg-slate-800 text-white hover:bg-slate-800">
                Template: {getNbaWheelUseCase(activeUseCaseId)?.label}
              </Badge>
              <Badge variant="secondary">
                {getNbaWheelUseCase(activeUseCaseId)?.config.teams.length} teams ready
              </Badge>
            </div>
          )}
            ...
            {data?.viewMode === "wheel" ? (
              <>
                ...
                >
                  {!isFullscreen && (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => window.dispatchEvent(new Event("open-nba-results"))}
                ...
                <NBAInputPanel 
                  ...
                  onOpenAchievements={() => setShowAchievements(true)}
                  onViewHistory={() => {
                    window.dispatchEvent(new Event("open-nba-results"))
                  }}
                  onOpenSettings={() => setShowSettings(true)}
                  onToggleFullscreen={() => setIsFullscreen((v) => !v)}
