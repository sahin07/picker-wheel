...
export function teamPickerSpokeMetadata(spokeId: TeamPickerSpokeId): Metadata {
  const spoke = getTeamPickerSpoke(spokeId)
  const url = teamSpokeUrl(spoke.path)
...
function TeamPickerSpokeJsonLd({ spokeId }: { spokeId: TeamPickerSpokeId }) {
  const spoke = getTeamPickerSpoke(spokeId)
  const url = teamSpokeUrl(spoke.path)
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: `${TEAM_PICKER_SITE_URL}${TEAM_PICKER_PATH}`,
      },
      {
        "@type": "ListItem",
        position: 3,
  ...
  const webPageSchema = {
    ...
    hasPart: [{ "@id": `${url}#article` }, { "@id": `${url}#tp-spoke-faq` }],
...
export default function TeamPickerSpokeRoute({
  spokeId,
}: {
  spokeId: TeamPickerSpokeId
}) {
  const spoke = getTeamPickerSpoke(spokeId)

  return (
    <>
      <TeamPickerSpokeJsonLd spokeId={spokeId} />
      <TeamPickerWheelApp
        deepLink={spoke.deepLink}
        shortTitle={spoke.shortTitle}
        toolSubtitle={spoke.heroIntro.slice(0, 110)}
        seoIntro={<TeamPickerSpokeSeoIntro spoke={spoke} />}
        seoSections={<TeamPickerSpokeSeoSections spoke={spoke} />}
      />
    </>
  )
}
