...
import {
  YES_NO_PICKER_USE_CASES,
  type YesNoPickerUseCase,
  type YesNoPickerUseCaseId,
} from "@/lib/yes-no-picker-use-cases"

interface YesNoPickerUseCasesProps {
  activeId?: YesNoPickerUseCaseId | null
  onSelectPreset: (id: YesNoPickerUseCaseId) => void
}

const USE_CASE_ICONS: Record<YesNoPickerUseCaseId, LucideIcon> = {
  "quick-decision": Zap,
  "yes-no-maybe": HelpCircle,
  elimination: Target,
  "ai-advice": Brain,
  "team-vote": Users,
  "date-night": Heart,
  "work-choice": Briefcase,
  "fair-coin": Coins,
  "group-consensus": Scale,
  "truth-check": Sparkles,
}

const ACCENT_STYLES: Record<
  YesNoPickerUseCase["accent"],
...
export function YesNoPickerUseCases({ activeId, onSelectPreset }: YesNoPickerUseCasesProps) {
  ...
  return (
    <section aria-labelledby="yes-no-picker-use-cases" className="mb-6 w-full">
      <div className="mb-3 text-center sm:text-left">
        <h2
          id="yes-no-picker-use-cases"
          className="text-lg font-bold text-slate-900 sm:text-xl"
        >
          Common Ways to Use a Yes or No Picker
        </h2>
        <p className="mt-1 text-sm text-slate-600">
          Swipe or tap a mode to load matching options and titles.
        </p>
      </div>
      ...
      <ul
        ...
        role="listbox"
        aria-label="Yes or No picker use cases"
      >
        {YES_NO_PICKER_USE_CASES.map((useCase) => {
