...
import { getAllJjkEntries } from "@/data/jjk-characters"
...
import type { ActionMode, JjkEntry, SpinResult } from "@/types/jjk-types"
...
export default function JjkWheelSection({
  ...
  const [winner, setWinner] = useState<JjkEntry | null>(null)
  ...
  const pendingWinner = useRef<JjkEntry | null>(null)
  ...
  const allEntries = useMemo(() => [...getAllJjkEntries(), ...(data?.customCharacters || [])], [data?.customCharacters])
  const activeEntries = useMemo(() => {
    ...
    const first = ordered.map((id) => byId.get(id)).filter(Boolean) as JjkEntry[]
  ...
  const updateData = useCallback((partial: Partial<JjkWheelData>) => {
    if (!wheel || !data) return
    const latest = useWheelManagerStore.getState().getCurrentWheel()?.data as JjkWheelData | undefined
    useWheelManagerStore.getState().updateWheelData("jjk-wheel", wheel.id, { ...(latest || data), ...partial })
  }, [data, wheel])
  ...
  useEffect(() => {
    ...
    window.addEventListener("open-jjk-results", open)
    return () => window.removeEventListener("open-jjk-results", open)
  }, [])
  ...
  const handleSpinComplete = useCallback(() => {
    ...
    updateData({
      ...
      spinHistory: [{
        id: `jjk-spin-${Date.now()}`, timestamp: new Date(), result: result.name,
  ...
  const addManualEntry = () => {
    ...
    const item: JjkEntry = {
      id: `jjk-custom-${Date.now()}`, name, emoji: "✨", category: ["main"], custom: true,
  ...
  return <div className={isFullscreen
    ...
    {confettiBurst > 0 && typeof document !== "undefined" && createPortal(
      ...
      <div ref={wheelBoxRef} className={`relative w-full max-w-[680px] overflow-visible ${!isSpinning && canvasItems.length ? "cursor-pointer" : ""}`}>
        <button type="button" className="block w-full" onClick={spin}
          disabled={isSpinning || !canvasItems.length} aria-label="Spin JJK character wheel">
    ...
    <Button type="button" onClick={spin} disabled={isSpinning || !canvasItems.length}
      ...
      {isSpinning
        ...
        : `SPIN JJK WHEEL (${activeEntries.length})`}
    </Button>
        ...
        <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
          {([
            ...
            <label key={mode.value}
              ...
              <input type="radio" name="jjkActionMode" value={mode.value} className="sr-only"
    ...
    {!canvasItems.length && <p className="text-sm text-slate-500">Enable at least one JJK entry to spin.</p>}
