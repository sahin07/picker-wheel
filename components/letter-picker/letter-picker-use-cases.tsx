...
import {
  LETTER_PICKER_USE_CASES,
  type LetterPickerUseCase,
  type LetterPickerUseCaseId,
} from "@/lib/letter-picker-use-cases"

interface LetterPickerUseCasesProps {
  activeId?: LetterPickerUseCaseId | null
  onSelectPreset: (id: LetterPickerUseCaseId) => void
}

const USE_CASE_ICONS: Record<LetterPickerUseCaseId, LucideIcon> = {
  classroom: GraduationCap,
  "alphabet-practice": BookOpen,
  phonics: Volume2,
  spelling: SpellCheck,
  "word-games": Puzzle,
  "creative-writing": PenLine,
  icebreaker: Users,
  "family-games": PartyPopper,
  esl: Languages,
  quiz: Lightbulb,
}

const ACCENT_STYLES: Record<
  LetterPickerUseCase["accent"],
...
export function LetterPickerUseCases({ activeId, onSelectPreset }: LetterPickerUseCasesProps) {
  ...
  return (
    <section aria-labelledby="letter-picker-use-cases" className="mb-6 w-full">
      <div className="mb-3 text-center sm:text-left">
        <h2
          id="letter-picker-use-cases"
          className="text-lg font-bold text-slate-900 sm:text-xl"
        >
          Common Uses
        </h2>
        <p className="mt-1 text-sm text-slate-600">
          Swipe or tap a mode to load matching letter sets and titles.
        </p>
      </div>
      ...
      <ul
        ...
        role="listbox"
        aria-label="Letter picker common uses"
      >
        {LETTER_PICKER_USE_CASES.map((useCase) => {
