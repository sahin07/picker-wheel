...
import { DEMON_SLAYER_WHEEL_DISCLAIMER, DEMON_SLAYER_WHEEL_PATH } from "@/lib/demon-slayer-wheel-seo"
import { getDemonSlayerSpokeSiblings, type DemonSlayerWheelSpokeSeo } from "@/lib/demon-slayer-wheel-spokes"

export function DemonSlayerWheelSpokeSeoIntro({ spoke }: { spoke: DemonSlayerWheelSpokeSeo }) {
...
export function DemonSlayerWheelSpokeSeoSections({ spoke }: { spoke: DemonSlayerWheelSpokeSeo }) {
  const siblings = getDemonSlayerSpokeSiblings(spoke)
  return <article className="mx-auto max-w-5xl space-y-12 px-1 pb-10 text-slate-700">
    <section><h2 className="font-spin-display mb-3 text-2xl font-bold text-slate-900">{spoke.articleTitle}</h2>
      ...
      <p className="mt-4 leading-relaxed text-slate-600">Use the search and toggles to refine the preset, then press Spin. Every enabled entry has equal odds. For unequal probabilities, use the <Link href="/weighted-wheel-spinner" className="font-semibold text-violet-700 hover:underline">Weighted Wheel Spinner</Link>. Return to the <Link href={DEMON_SLAYER_WHEEL_PATH} className="font-semibold text-violet-700 hover:underline">Demon Slayer Spin Wheel hub</Link> for the complete guide.</p>
    </section>
    <section><h2 className="font-spin-display mb-4 text-2xl font-bold text-slate-900">Related Demon Slayer Wheels</h2>
    ...
    <p className="rounded-xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-900">{DEMON_SLAYER_WHEEL_DISCLAIMER}</p>
  </article>
}
