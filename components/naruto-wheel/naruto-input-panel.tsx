...
import { NARUTO_WHEEL_USE_CASES, type NarutoWheelUseCaseId } from "@/lib/naruto-wheel-use-cases"
...
import type { ActionMode, DisplayMode, NarutoCategory, NarutoEntry } from "@/types/naruto-types"
import type { WheelSettings } from "@/types/settings"
import { NarutoAiTab } from "./naruto-ai-tab"

type Tab = "inputs" | "text" | "style" | "other"
type InputsSubTab = "manual" | "ai" | "stats"
type CategoryFilter = "all" | NarutoCategory
...
const CATEGORY_FILTERS: { id: CategoryFilter; label: string }[] = [
  ...
  { id: "hokage", label: "Hokage" },
  { id: "uchiha", label: "Uchiha" },
  { id: "jinchuriki", label: "Jinchuriki" },
  { id: "kage", label: "Kage" },
...
type Props = {
  ...
  onApplyTemplate: (id: NarutoWheelUseCaseId) => void
  onHideInputs?: () => void
  onOpenSettings?: () => void
...
export default function NarutoInputPanel({
  ...
  const [previewEntry, setPreviewEntry] = useState<NarutoEntry | null>(null)
  ...
  const selected = Array.isArray(data?.selectedCharacters) ? data.selectedCharacters : []
  const custom = Array.isArray(data?.customCharacters) ? data.customCharacters : []
  const favorites = Array.isArray(data?.favoriteCharacters) ? data.favoriteCharacters : []
  const comparison = Array.isArray(data?.comparisonCharacters) ? data.comparisonCharacters : []
  const showTitle = data?.showTitle !== false
  const displayMode = data?.displayMode || "emoji-name"
  const entries = useMemo(() => [...getAllNarutoEntries(), ...custom], [custom])
  ...
  const setData = (partial: Partial<NarutoWheelData>) => {
    if (!wheel || !data) return
    useWheelManagerStore.getState().updateWheelData("naruto-wheel", wheel.id, { ...data, ...partial })
  }
  ...
  const addCustom = (preset?: Partial<NarutoEntry>) => {
    ...
    const item: NarutoEntry = {
      id: preset?.id || `naruto-custom-${Date.now()}`, name,
  ...
  const updateCustom = (id: string, patch: Partial<NarutoEntry>) =>
  ...
  const toggleFavorite = (entry: NarutoEntry) => {
  ...
  const toggleComparison = (entry: NarutoEntry) => {
  ...
  >
    <div className="flex shrink-0 items-center justify-between gap-1 border-b bg-orange-50/70 px-2 py-2 sm:gap-2 sm:px-3">
      <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
        <p className="truncate text-xs font-semibold text-slate-800 sm:text-sm">Naruto Wheel Controls</p>
        <span className="shrink-0 rounded-full bg-orange-100 px-1.5 py-0.5 text-[10px] font-medium text-orange-700 sm:px-2 sm:text-xs">
      ...
      <div className="flex shrink-0 items-center gap-0 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {onViewHistory && (
          <Button type="button" variant="ghost" size="sm" className="relative h-7 w-7 p-0 sm:h-8 sm:w-8"
            ...
            {resultsCount > 0 && (
              <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-orange-600 px-0.5 text-[10px] text-white">
                {resultsCount}
              </span>
            )}
          </Button>
        ...
        <SlicesManageMenu settings={settings as unknown as WheelSettings}
          ...
          onEqualize={() => showToast("Naruto entries always use equal odds", "success")}
          onDeleteBlanks={() => {
      ...
      {TABS.map((item) => (
        <button key={item.id} type="button"
          ...
          className={`flex min-w-[4.5rem] flex-1 flex-col items-center gap-1 px-2 py-2.5 text-xs font-medium ${
            tab === item.id ? "border-b-2 border-orange-600 bg-orange-50 text-orange-800" : "text-slate-500"
          }`}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-2.5 sm:p-3">
      {tab === "inputs" && <div className="space-y-4">
        ...
        {inputsSubTab === "manual" && <>
          <div><Label>Template</Label>
            <Select onValueChange={(value) => onApplyTemplate(value as NarutoWheelUseCaseId)}>
              <SelectTrigger className="mt-1"><SelectValue placeholder="Choose a Naruto template" /></SelectTrigger>
              <SelectContent>{NARUTO_WHEEL_USE_CASES.map((item) =>
          ...
          <div className="rounded-lg border p-3">
            ...
            <div className="rounded-lg border border-orange-200 bg-orange-50 p-2 text-xs text-orange-950">
      ...
      {tab === "text" && <div className="space-y-3">
        <div>
          <Label className="text-sm font-medium text-slate-700">Naruto list</Label>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" variant="outline" onClick={() => {
            ...
            anchor.href = url
            anchor.download = "naruto-wheel.txt"
            anchor.click()
            ...
            {([
              ...
              <label key={opt.value}
                className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm ${
                  displayMode === opt.value ? "border-orange-400 bg-orange-50" : "border-slate-200 hover:bg-slate-50"
                }`}>
                <input type="radio" name="naruto-display" value={opt.value}
          ...
          <div className="grid grid-cols-2 gap-2">
            {LETTER_COLOR_PALETTES.map((palette) => (
              <button key={palette.name} type="button" onClick={() => applyPalette(palette.colors)}
                className="rounded-lg border border-slate-200 p-2 text-left hover:border-orange-300 hover:bg-orange-50/40">
      ...
      {tab === "other" && <div className="space-y-4">
        ...
        <SidebarOtherOptions toolLabel="Naruto entries" resultsCount={resultsCount || data?.recentResults?.length || 0}
          exportFileName="naruto-wheel.txt" exportText={exportText}
          ...
          onRemoveDuplicates={removeDuplicates}
          onViewResults={onViewHistory || (() => window.dispatchEvent(new Event("open-naruto-results")))}
    ...
    <Dialog open={showFavorites} onOpenChange={setShowFavorites}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Saved Naruto entries for quick access.</DialogDescription>
        </DialogHeader>
    ...
    <Dialog open={showComparison} onOpenChange={setShowComparison}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Compare up to 4 Naruto entries side by side.</DialogDescription>
        </DialogHeader>
      ...
      <DialogContent>
        ...
        {previewEntry && (
          <div className="space-y-3">
            ...
            {previewEntry.village && <p className="text-sm text-slate-600">Village: {previewEntry.village}</p>}
            {previewEntry.clan && <p className="text-sm text-slate-600">Clan: {previewEntry.clan}</p>}
            {previewEntry.rank && <p className="text-sm text-slate-600">Rank: {previewEntry.rank}</p>}
            {previewEntry.blurb && <p className="text-sm text-slate-600">{previewEntry.blurb}</p>}
            <div className="flex flex-wrap gap-2">
