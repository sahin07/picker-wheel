...
import type { NarutoWheelDeepLink } from "@/lib/naruto-wheel-spokes"
import { NARUTO_WHEEL_SHORT_TITLE } from "@/lib/naruto-wheel-seo"
import {
  applyNarutoWheelUseCase, getNarutoWheelUseCase, narutoWheelUseCaseFromTemplate,
  type NarutoWheelUseCaseId,
} from "@/lib/naruto-wheel-use-cases"
import {
...
import type { ActionMode } from "@/types/naruto-types"
import NarutoInputPanel from "./naruto-input-panel"
import { NarutoWheelPopularTemplates } from "./naruto-wheel-popular-templates"
import NarutoWheelSection from "./naruto-wheel-section"
import { NarutoFunModes } from "./naruto-fun-modes"

export type NarutoWheelAppProps = {
  seoIntro?: ReactNode
...
function NarutoWheelAppInner({ seoIntro, seoSections, shortTitle, toolSubtitle, deepLink, showFooter = true }: NarutoWheelAppProps) {
  ...
  const [activeUseCaseId, setActiveUseCaseId] = useState<NarutoWheelUseCaseId | null>(deepLink?.useCaseId ?? null)
  const lastApplied = useRef<NarutoWheelUseCaseId | null>(null)
  ...
  const applyPreset = useCallback((id: NarutoWheelUseCaseId) => {
    if (!getNarutoWheelUseCase(id) || !applyNarutoWheelUseCase(id)) return
  ...
  useEffect(() => {
    ...
    const initialize = () => {
      ...
      store.setCurrentTool("naruto-wheel")
      ...
      const latest = store.getCurrentWheel()?.data as NarutoWheelData | undefined
  ...
  const onSearchParams = useCallback(
    (params: URLSearchParams) => {
      ...
      const preset = narutoWheelUseCaseFromTemplate(params.get("template") || params.get("mode"))
  ...
  const syncActionMode = useCallback((mode: ActionMode) => {
    ...
    if (current) updateWheelData("naruto-wheel", current.id, { ...(current.data as NarutoWheelData), actionMode: mode })
    if (mode !== "manual") {
  ...
  const selectTheme = useCallback((themeId: string) => {
    ...
    const current = getCurrentWheel()
    if (current) updateWheelData("naruto-wheel", current.id, { ...(current.data as NarutoWheelData), currentTheme: themeId })
  }, [getCurrentWheel, updateWheelData])

  const handleSpinCompleted = useCallback(() => {
    const current = getCurrentWheel()
    if (!current) return
    const wheelData = current.data as NarutoWheelData
    ...
    window.setTimeout(() => {
      ...
      const latest = latestWheel.data as NarutoWheelData
      ...
      if (newlyCompleted.length > 0 || newlyUnlockedThemes.length > 0) {
        updateWheelData("naruto-wheel", latestWheel.id, {
          ...latest,
  ...
  const resetWheelForGame = useCallback(() => {
    ...
    updateWheelData("naruto-wheel", current.id, {
      ...(current.data as NarutoWheelData),
      totalSpins: 0,
      spinHistory: [],
      recentResults: [],
      selectedResult: null,
  ...
  if (!mounted) {
    return (
      <>
        <div className="min-h-[720px] bg-orange-100" aria-busy="true" />
  ...
  return (
    ...
    style={{ backgroundColor: settings.appearance.backgroundColor || "#ffedd5",
    ...
    <main className="w-full min-w-0 overflow-x-hidden px-3 py-4 sm:px-6 sm:py-8 lg:px-8">
      ...
      <div className="mb-4 text-center">
        <ToolPageTitle title={shortTitle ?? NARUTO_WHEEL_SHORT_TITLE} toolType="naruto-wheel" />
        <p className="text-gray-600">{toolSubtitle ?? "Pick a random Naruto character with equal odds"}</p>
      </div>
      <NarutoWheelPopularTemplates />
      <NarutoFunModes onApplyTemplate={applyPreset} />
      {activeUseCaseId && getNarutoWheelUseCase(activeUseCaseId) && <div className="mb-4 flex justify-center sm:justify-start">
        <span className="rounded-full bg-orange-100 px-3 py-1 text-sm font-semibold text-orange-800">
          Template: {getNarutoWheelUseCase(activeUseCaseId)!.label}</span></div>}
      </>}
      ...
      <div id="naruto-wheel-tool" className="mb-6 grid min-w-0 items-start gap-4 sm:mb-8 sm:gap-6 lg:grid-cols-3 lg:gap-8">
        <div
          ref={leftColRef}
        ...
        >
          {!isFullscreen && <Button type="button" variant="outline" size="sm"
            onClick={() => window.dispatchEvent(new Event("open-naruto-results"))}
            className="absolute left-2 top-2 z-10 border-orange-500 bg-white text-xs text-orange-800 shadow-sm sm:left-4 sm:top-4">
          ...
          <div className={isFullscreen ? undefined : "pt-8"}><NarutoWheelSection currentTheme={currentTheme}
            themes={themes}
        ...
        {showInputs && !isFullscreen && (
          ...
          onOpenAchievements={() => setShowAchievements(true)}
          onOpenAnalytics={() => setShowAnalytics(true)}
          onViewHistory={() => window.dispatchEvent(new Event("open-naruto-results"))}
    ...
    <PickerWheelAnalyticsDisplay
      analytics={(() => {
        const wheelData = getCurrentWheel()?.data as NarutoWheelData | undefined
...
export default function NarutoWheelApp(props: NarutoWheelAppProps) {
  return <NarutoWheelAppInner {...props} />
}
