...
import { DTI_WHEEL_SHORT_TITLE } from "@/lib/dti-wheel-seo"
import type { DtiWheelDeepLink } from "@/lib/dti-wheel-spokes"
import {
  applyDtiWheelUseCase,
  dtiWheelUseCaseFromTemplate,
  getDtiWheelUseCase,
...
import { DtiWheelPopularTemplates } from "./dti-wheel-popular-templates"

const EMPTY_ENTRIES: any[] = []
const EMPTY_RESULTS: any[] = []

export type DtiWheelAppProps = {
  seoIntro?: ReactNode
...
function DtiWheelAppInner({
  ...
  const [activeUseCaseId, setActiveUseCaseId] = useState<DtiWheelUseCaseId | null>(
    deepLink?.useCaseId ?? null,
  )
  const [mounted, setMounted] = useState(false)
  const [sidebarMaxHeight, setSidebarMaxHeight] = useState<number | null>(null)
  const lastApplied = useRef<DtiWheelUseCaseId | null>(null)
  ...
  const viewMode = useWheelManagerStore((state) => {
    const wheel =
      state.wheelsByTool["dti-wheel"]?.find((item) => item.id === state.currentWheelId) ||
      state.wheelsByTool["dti-wheel"]?.[0]
  ...
  const applyPreset = useCallback((id: DtiWheelUseCaseId) => {
    if (!getDtiWheelUseCase(id) || !applyDtiWheelUseCase(id)) return
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      setCurrentTool("dti-wheel")
      let wheel = getCurrentWheel()
      if (!wheel) {
        createNewWheel("dti-wheel", "My DTI Outfit Wheel")
    ...
    initialize().catch((error) => console.error("Error initializing DTI wheel:", error))
  ...
  const onSearchParams = useCallback(
    (params: URLSearchParams) => {
      ...
      const preset = dtiWheelUseCaseFromTemplate(params.get("template"))
  ...
  const syncActionMode = useCallback(
    (mode: "normal" | "elimination" | "manual") => {
      ...
      if (wheel) {
        updateWheelData("dti-wheel", wheel.id, {
  ...
  const resetWheelForGame = useCallback(() => {
    ...
    updateWheelData("dti-wheel", current.id, {
  ...
  const handleSpinCompleted = useCallback(() => {
    ...
    window.setTimeout(() => {
      ...
      if (newlyCompleted.length > 0 || newlyUnlockedThemes.length > 0) {
        updateWheelData("dti-wheel", latestWheel.id, {
          ...latest,
  ...
  if (!mounted) {
    return (
      <>
        <div className="min-h-[720px] bg-pink-50" aria-busy="true" />
              ...
              <>
                ...
                >
                  {!isFullscreen && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.dispatchEvent(new Event("open-dti-results"))}
                  ...
                  <div className={isFullscreen ? undefined : "pt-8"}>
                    <FortuneWheelSection
                      toolType="dti-wheel"
                      resultsEventName="open-dti-results"
                ...
                {showInputs && !isFullscreen && (
                  <div className="min-w-0 self-start lg:col-span-1">
                    <FortuneInputPanel
                      toolType="dti-wheel"
                      exportFileName="dti-wheel.txt"
                      controlsTitle="DTI Controls"
                      toolLabel="DTI themes"
                      resultsEventName="open-dti-results"
                      equalOddsToast="DTI themes always use equal odds"
                      confettiHint="Celebrate winning outfit themes"
...
export default function DtiWheelApp(props: DtiWheelAppProps) {
  return <DtiWheelAppInner {...props} />
}
