...
import { DEMON_SLAYER_WHEEL_USE_CASES, type DemonSlayerWheelUseCaseId } from "@/lib/demon-slayer-wheel-use-cases"
...
import type { ActionMode, DisplayMode, DemonSlayerCategory, DemonSlayerEntry } from "@/types/demon-slayer-types"
import type { WheelSettings } from "@/types/settings"
import { DemonSlayerAiTab } from "./demon-slayer-ai-tab"

type Tab = "inputs" | "text" | "style" | "other"
type InputsSubTab = "manual" | "ai" | "stats"
type CategoryFilter = "all" | DemonSlayerCategory
...
const CATEGORY_FILTERS: { id: CategoryFilter; label: string }[] = [
  ...
  { id: "upper_rank", label: "Upper Rank" },
  { id: "lower_rank", label: "Lower Rank" },
  { id: "corps", label: "Corps" },
  { id: "breathing", label: "Breathing" },
  { id: "nichirin", label: "Nichirin" },
]
...
type Props = {
  ...
  onApplyTemplate: (id: DemonSlayerWheelUseCaseId) => void
  onHideInputs?: () => void
  onOpenSettings?: () => void
...
export default function DemonSlayerInputPanel({
  ...
  const [previewEntry, setPreviewEntry] = useState<DemonSlayerEntry | null>(null)
  ...
  const entries = useMemo(() => [...getAllDemonSlayerEntries(), ...custom], [custom])
  ...
  const setData = (partial: Partial<DemonSlayerWheelData>) => {
    if (!wheel || !data) return
    useWheelManagerStore.getState().updateWheelData("demon-slayer-wheel", wheel.id, { ...data, ...partial })
  }
  ...
  const addCustom = (preset?: Partial<DemonSlayerEntry>) => {
    ...
    const item: DemonSlayerEntry = {
      id: preset?.id || `demon-slayer-custom-${Date.now()}`, name,
  ...
  const updateCustom = (id: string, patch: Partial<DemonSlayerEntry>) =>
  ...
  const toggleFavorite = (entry: DemonSlayerEntry) => {
  ...
  const toggleComparison = (entry: DemonSlayerEntry) => {
  ...
  >
    <div className="flex shrink-0 items-center justify-between gap-1 border-b bg-violet-50/70 px-2 py-2 sm:gap-2 sm:px-3">
      <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
        <p className="truncate text-xs font-semibold text-slate-800 sm:text-sm">Demon Slayer Wheel Controls</p>
      ...
      <div className="flex shrink-0 items-center gap-0 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <SlicesManageMenu settings={settings as unknown as WheelSettings}
          ...
          onEqualize={() => showToast("Demon Slayer entries always use equal odds", "success")}
          onDeleteBlanks={() => {
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-2.5 sm:p-3">
      {tab === "inputs" && <div className="space-y-4">
        ...
        {inputsSubTab === "manual" && <>
          <div><Label>Template</Label>
            <Select onValueChange={(value) => onApplyTemplate(value as DemonSlayerWheelUseCaseId)}>
              <SelectTrigger className="mt-1"><SelectValue placeholder="Choose a Demon Slayer template" /></SelectTrigger>
              <SelectContent>{DEMON_SLAYER_WHEEL_USE_CASES.map((item) =>
      ...
      {tab === "text" && <div className="space-y-3">
        <div>
          <Label className="text-sm font-medium text-slate-700">Demon Slayer list</Label>
        ...
        <Textarea value={bulkText} onChange={(event) => setBulkText(event.target.value)} rows={12}
          placeholder={"Tanjiro Kamado\nGiyu Tomioka\nNezuko Kamado"} className="font-mono text-sm" />
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" variant="outline" onClick={() => {
            ...
            anchor.href = url
            anchor.download = "demon-slayer-wheel.txt"
            anchor.click()
          ...
          <div className="grid gap-2">
            {([
              ...
              <label key={opt.value}
                ...
                <input type="radio" name="demon-slayer-display" value={opt.value}
      ...
      {tab === "other" && <div className="space-y-4">
        ...
        <SidebarOtherOptions toolLabel="Demon Slayer entries" resultsCount={resultsCount || data?.recentResults?.length || 0}
          exportFileName="demon-slayer-wheel.txt" exportText={exportText}
          ...
          onRemoveDuplicates={removeDuplicates}
          onViewResults={onViewHistory || (() => window.dispatchEvent(new Event("open-demon-slayer-results")))}
    ...
    <Dialog open={showFavorites} onOpenChange={setShowFavorites}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Saved Demon Slayer entries for quick access.</DialogDescription>
        </DialogHeader>
    ...
    <Dialog open={showComparison} onOpenChange={setShowComparison}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Compare up to 4 Demon Slayer entries side by side.</DialogDescription>
        </DialogHeader>
      ...
      <DialogContent>
        ...
        {previewEntry && (
          <div className="space-y-3">
            ...
            {previewEntry.breathingStyle && <p className="text-sm text-slate-600">Breathing: {previewEntry.breathingStyle}</p>}
