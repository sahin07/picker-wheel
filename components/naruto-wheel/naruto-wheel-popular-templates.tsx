...
const ACCENTS: Record<string, string> = {
  orange: "border-orange-200 from-orange-50 hover:border-orange-400",
...
export function NarutoWheelPopularTemplates() {
  const pathname = usePathname()
  return <section id="naruto-popular" className="mb-6 w-full scroll-mt-24" aria-labelledby="naruto-popular-heading">
    <div className="mb-3 text-center sm:text-left"><h2 id="naruto-popular-heading" className="text-lg font-bold text-slate-900 sm:text-xl">
      Popular Naruto Wheels</h2>
      <p className="mt-1 text-sm text-slate-600">Character, Akatsuki, Hokage, Uchiha, jutsu, village, team, and quiz-intent templates.</p></div>
    <ul className="flex snap-x gap-3 overflow-x-auto pb-2 [scrollbar-width:none]">
      {NARUTO_WHEEL_POPULAR_SPOKE_LINKS.map((item) => {
        ...
        return <li key={item.href} className="w-[220px] shrink-0 snap-center"><Link href={item.href}
          ...
          className={`flex h-full gap-3 rounded-xl border bg-gradient-to-br to-white p-3 transition-all ${ACCENTS[item.accent]} ${active ? "ring-2 ring-orange-300 shadow-md" : ""}`}>
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white shadow-sm"><CircleDot className="h-5 w-5 text-orange-700" /></span>
