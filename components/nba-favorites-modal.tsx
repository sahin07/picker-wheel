...
interface NBAFavoritesModalProps {
  isOpen: boolean
  onClose: () => void
}

export default function NBAFavoritesModal({ isOpen, onClose }: NBAFavoritesModalProps) {
  ...
  const data = (wheel?.data as NBAWheelData) ?? ({ favoriteTeams: [] } as Pick<NBAWheelData, "favoriteTeams">)
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div className="text-center">
              <div className="text-lg font-bold text-purple-600">
                {favoriteTeams.filter(t => t.league === 'Eastern').length}
              </div>
              <div className="text-xs text-gray-600">Eastern Conference</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-orange-600">
                {favoriteTeams.filter(t => t.league === 'Western').length}
              </div>
              <div className="text-xs text-gray-600">Western Conference</div>
            </div>
          </div>
        </div>
      </DialogContent>
