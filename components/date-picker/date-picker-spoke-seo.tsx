...
import {
  getDateSpokeSiblings,
  type DatePickerSpokeSeo,
} from "@/lib/date-picker-spokes"

export function DatePickerSpokeSeoIntro({ spoke }: { spoke: DatePickerSpokeSeo }) {
  return (
    <section
      aria-labelledby="date-spoke-seo-h1"
      className="mx-auto mb-10 max-w-5xl scroll-mt-24 border-t border-slate-200 px-1 pt-10 text-slate-700"
    >
      <h1
        id="date-spoke-seo-h1"
...
export function DatePickerSpokeSeoSections({ spoke }: { spoke: DatePickerSpokeSeo }) {
  const siblings = getDateSpokeSiblings(spoke)
  const toc = [
    { id: "dp-spoke-guide", label: "Guide" },
    { id: "dp-spoke-unique", label: spoke.uniqueSection.title },
    { id: "dp-spoke-cluster", label: "Related date wheels" },
    { id: "dp-spoke-faq", label: "FAQ" },
  ]

  return (
    <div className="mx-auto max-w-5xl space-y-16 px-1 pb-8 pt-4 text-slate-700">
      <article id="date-spoke-article" className="border-t border-slate-200 pt-12">
        ...
        <section id="dp-spoke-guide" aria-labelledby="dp-spoke-guide-heading" className="scroll-mt-24">
          <h2
            id="dp-spoke-guide-heading"
          ...
          <p className="mt-4 text-base leading-relaxed text-slate-600">
            The template above is already loaded. Adjust weekday filters, formats, or open the main{" "}
            <Link
              href={DATE_PICKER_PATH}
              className="text-emerald-700 underline-offset-2 hover:underline"
            >
        ...
        <section
          id="dp-spoke-unique"
          aria-labelledby="dp-spoke-unique-heading"
          className="mt-12 scroll-mt-24"
        >
          <h2
            id="dp-spoke-unique-heading"
      ...
      <section
        id="dp-spoke-cluster"
        aria-labelledby="dp-spoke-cluster-heading"
        className="scroll-mt-24 border-t border-slate-200 pt-12"
      >
        <h2
          id="dp-spoke-cluster-heading"
          className="font-spin-display mb-3 text-2xl font-bold text-slate-900 sm:text-3xl"
        >
          Related Date Wheels
        </h2>
      ...
      <section
        id="dp-spoke-faq"
        aria-labelledby="dp-spoke-faq-heading"
        className="scroll-mt-24 border-t border-slate-200 pt-12"
      >
        <h2
          id="dp-spoke-faq-heading"
