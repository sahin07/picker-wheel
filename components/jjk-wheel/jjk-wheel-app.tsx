...
import type { JjkWheelDeepLink } from "@/lib/jjk-wheel-spokes"
import { JJK_WHEEL_SHORT_TITLE } from "@/lib/jjk-wheel-seo"
import {
  applyJjkWheelUseCase, getJjkWheelUseCase, jjkWheelUseCaseFromTemplate,
  type JjkWheelUseCaseId,
} from "@/lib/jjk-wheel-use-cases"
import {
...
import type { ActionMode } from "@/types/jjk-types"
import JjkInputPanel from "./jjk-input-panel"
import { JjkWheelPopularTemplates } from "./jjk-wheel-popular-templates"
import JjkWheelSection from "./jjk-wheel-section"

export type JjkWheelAppProps = {
  seoIntro?: ReactNode
...
function JjkWheelAppInner({ seoIntro, seoSections, shortTitle, toolSubtitle, deepLink, showFooter = true }: JjkWheelAppProps) {
  ...
  const [activeUseCaseId, setActiveUseCaseId] = useState<JjkWheelUseCaseId | null>(deepLink?.useCaseId ?? null)
  const lastApplied = useRef<JjkWheelUseCaseId | null>(null)
  ...
  }, [mounted, showInputs, isFullscreen, actionMode, currentTheme, data?.selectedIds?.length])

  const applyPreset = useCallback((id: JjkWheelUseCaseId) => {
    if (!getJjkWheelUseCase(id) || !applyJjkWheelUseCase(id)) return
  ...
  useEffect(() => {
    ...
    const initialize = () => {
      ...
      store.setCurrentTool("jjk-wheel")
      let current = store.getCurrentWheel()
      if (!current) {
        store.createNewWheel("jjk-wheel", "JJK Spin Wheel picker")
        current = store.getCurrentWheel()
      }
      const preset = deepLink?.useCaseId || jjkWheelUseCaseFromTemplate(searchParams.get("template"))
      if (preset) applyPreset(preset)
      const latest = store.getCurrentWheel()?.data as JjkWheelData | undefined
  ...
  const syncActionMode = useCallback((mode: ActionMode) => {
    ...
    if (current) updateWheelData("jjk-wheel", current.id, { ...(current.data as JjkWheelData), actionMode: mode })
    if (mode !== "manual") {
  ...
  const selectTheme = useCallback((themeId: string) => {
    ...
    const current = getCurrentWheel()
    if (current) updateWheelData("jjk-wheel", current.id, { ...(current.data as JjkWheelData), currentTheme: themeId })
  }, [getCurrentWheel, updateWheelData])

  const handleSpinCompleted = useCallback(() => {
    const current = getCurrentWheel()
    if (!current) return
    const wheelData = current.data as JjkWheelData
    ...
    window.setTimeout(() => {
      ...
      const latest = latestWheel.data as JjkWheelData
      ...
      if (newlyCompleted.length > 0 || newlyUnlockedThemes.length > 0) {
        updateWheelData("jjk-wheel", latestWheel.id, {
          ...latest,
  ...
  const resetWheelForGame = useCallback(() => {
    ...
    updateWheelData("jjk-wheel", current.id, {
      ...(current.data as JjkWheelData),
      totalSpins: 0,
      spinHistory: [],
      recentResults: [],
      selectedResult: null,
  ...
  return <ToastProvider><div className={`min-h-screen transition-colors ${isFullscreen ? "fixed inset-0 z-50 overflow-auto" : ""}`}
    ...
    <main className="w-full min-w-0 overflow-x-hidden px-3 py-4 sm:px-6 sm:py-8 lg:px-8">
      {!isFullscreen && <><div className="mb-4 text-center">
        <ToolPageTitle title={shortTitle ?? JJK_WHEEL_SHORT_TITLE} toolType="jjk-wheel" />
        <p className="text-gray-600">{toolSubtitle ?? "Pick a random Jujutsu Kaisen character with equal odds"}</p>
      </div>
      <JjkWheelPopularTemplates />
      {activeUseCaseId && getJjkWheelUseCase(activeUseCaseId) && <div className="mb-4 flex justify-center sm:justify-start">
        <span className="rounded-full bg-violet-100 px-3 py-1 text-sm font-semibold text-violet-800">
          Template: {getJjkWheelUseCase(activeUseCaseId)!.label}</span></div>}</>}
      ...
      <div id="jjk-wheel-tool" className="mb-6 grid min-w-0 items-start gap-4 sm:mb-8 sm:gap-6 lg:grid-cols-3 lg:gap-8">
        <div
          ref={leftColRef}
        ...
        >
          {!isFullscreen && <Button type="button" variant="outline" size="sm"
            onClick={() => window.dispatchEvent(new Event("open-jjk-results"))}
          ...
          <div className={isFullscreen ? undefined : "pt-8"}><JjkWheelSection currentTheme={currentTheme}
            themes={themes}
        ...
        {showInputs && !isFullscreen && (
          ...
          onOpenAchievements={() => setShowAchievements(true)}
          onOpenAnalytics={() => setShowAnalytics(true)}
          onViewHistory={() => window.dispatchEvent(new Event("open-jjk-results"))}
    ...
    <PickerWheelAnalyticsDisplay
      analytics={(() => {
        const wheelData = getCurrentWheel()?.data as JjkWheelData | undefined
...
export default function JjkWheelApp(props: JjkWheelAppProps) {
  return <Suspense fallback={<div className="min-h-screen bg-violet-100" />}>
    <JjkWheelAppInner {...props} />
  </Suspense>
}
