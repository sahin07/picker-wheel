...
import { THEME_PICKER_SHORT_TITLE } from "@/lib/theme-picker-seo"
import type { ThemePickerDeepLink } from "@/lib/theme-picker-spokes"
import {
  applyThemePickerUseCase,
  themePickerUseCaseFromTemplate,
...
import FortuneInputPanel from "@/components/fortune-wheel/fortune-input-panel"
import FortuneWheelSection from "@/components/fortune-wheel/fortune-wheel-section"
import { ThemePickerPopularTemplates } from "./theme-picker-popular-templates"

const EMPTY_ENTRIES: any[] = []
const EMPTY_RESULTS: any[] = []

export type ThemePickerAppProps = {
  seoIntro?: ReactNode
...
function ThemePickerAppInner({
  ...
  const [activeUseCaseId, setActiveUseCaseId] = useState<ThemePickerUseCaseId | null>(
    deepLink?.useCaseId ?? null,
  )
  const [mounted, setMounted] = useState(false)
  const [sidebarMaxHeight, setSidebarMaxHeight] = useState<number | null>(null)
  const lastApplied = useRef<ThemePickerUseCaseId | null>(null)
  ...
  const viewMode = useWheelManagerStore((state) => {
    const wheel =
      state.wheelsByTool["theme-picker-wheel"]?.find((item) => item.id === state.currentWheelId) ||
      state.wheelsByTool["theme-picker-wheel"]?.[0]
  ...
  const applyPreset = useCallback((id: ThemePickerUseCaseId) => {
    if (!getThemePickerUseCase(id) || !applyThemePickerUseCase(id)) return
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      setCurrentTool("theme-picker-wheel")
      let wheel = getCurrentWheel()
      if (!wheel) {
        createNewWheel("theme-picker-wheel", "My Theme Wheel")
    ...
    initialize().catch((error) => console.error("Error initializing Theme Picker wheel:", error))
  ...
  const onSearchParams = useCallback(
    (params: URLSearchParams) => {
      ...
      const preset = themePickerUseCaseFromTemplate(params.get("template"))
  ...
  const syncActionMode = useCallback(
    (mode: "normal" | "elimination" | "manual") => {
      ...
      if (wheel) {
        updateWheelData("theme-picker-wheel", wheel.id, {
  ...
  const resetWheelForGame = useCallback(() => {
    ...
    updateWheelData("theme-picker-wheel", current.id, {
  ...
  const handleSpinCompleted = useCallback(() => {
    ...
    window.setTimeout(() => {
      ...
      if (newlyCompleted.length > 0 || newlyUnlockedThemes.length > 0) {
        updateWheelData("theme-picker-wheel", latestWheel.id, {
          ...latest,
  ...
  if (!mounted) {
    return (
      <>
        <div className="min-h-[720px] bg-teal-50" aria-busy="true" />
              ...
              <>
                ...
                >
                  {!isFullscreen && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.dispatchEvent(new Event("open-theme-picker-results"))}
                  ...
                  <div className={isFullscreen ? undefined : "pt-8"}>
                    <FortuneWheelSection
                      toolType="theme-picker-wheel"
                      resultsEventName="open-theme-picker-results"
                ...
                {showInputs && !isFullscreen && (
                  <div className="min-w-0 self-start lg:col-span-1">
                    <FortuneInputPanel
                      toolType="theme-picker-wheel"
                      exportFileName="theme-picker-wheel.txt"
                      controlsTitle="Theme Controls"
                      toolLabel="themes"
                      resultsEventName="open-theme-picker-results"
                      equalOddsToast="themes always use equal odds"
                      confettiHint="Celebrate the winning theme"
...
export default function ThemePickerWheelApp(props: ThemePickerAppProps) {
  return <ThemePickerAppInner {...props} />
}
