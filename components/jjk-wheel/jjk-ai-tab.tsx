...
type Props = {
  ...
  customEntries?: JjkEntry[]
  onAddCustom?: (entry: JjkEntry) => void
  recentResults?: JjkEntry[]
  totalSpins?: number
}

const QUICK_QUESTIONS = [
  "Who should I main for a beginner JJK challenge?",
  "Build a balanced sorcerer vs cursed spirit draft",
  "Suggest Domains for a Domain Expansion challenge",
  "Pick a special grade character for a tough run",
]

const GENERATOR_PRESETS: { label: string; category?: JjkCategory; size?: number }[] = [
  { label: "Students", category: "student" },
...
export function JjkAiTab({
  ...
  const catalog = useMemo(() => [...getAllJjkEntries(), ...customEntries], [customEntries])

  const applyCategory = (category: JjkCategory) => {
  ...
  const submitQuery = async (text = query) => {
    ...
    try {
      ...
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          prompt: `You are a Jujutsu Kaisen wheel assistant. Play style: ${playStyle}. Active wheel entries: ${names.join(", ") || "none"}. Recent winners: ${recentResults.map((item) => item.name).join(", ") || "none"}. Total spins: ${totalSpins}. User question: ${prompt}`,
          context: "jjk-wheel",
        }),
      })
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[10px] text-violet-600">JJK wheel helper</p>
        </div>
      </div>
      ...
      {mode === "generator" && (
        <div className="space-y-3">
          <p className="text-xs text-slate-500">Build a ready-made JJK wheel set in one tap.</p>
          ...
          <div className="space-y-2 rounded-lg border p-3">
            ...
            <Input value={query} onChange={(event) => setQuery(event.target.value)}
              placeholder="e.g. Tokyo Jujutsu High students" />
            <Button type="button" size="sm" className="w-full" disabled={loading}
              onClick={() => void submitQuery(query || "Suggest a themed JJK wheel set")}>
              Generate
            </Button>
          </div>
          {response && <div className="rounded-lg border bg-violet-50 p-3 text-xs text-violet-950 whitespace-pre-wrap">{response}</div>}
