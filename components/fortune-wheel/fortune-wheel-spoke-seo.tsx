...
export function FortuneWheelSpokeSeoIntro({ spoke }: { spoke: FortuneWheelSpokeSeo }) {
  return (
    <section
      aria-labelledby="fortune-spoke-h1"
  ...
  return (
    <div className="mx-auto max-w-5xl px-1 pb-8 pt-4 text-slate-700">
      <article className="border-t border-slate-200 pt-12">
        ...
        <SpokeTipsSection tips={spoke.tips} id="fortune-spoke-tips" accentClass="marker:text-violet-700" />
        <SpokeIntentRelatedSection
          currentPath={spoke.path}
          id="fortune-spoke-intent"
          accentHover="hover:border-violet-300 hover:bg-violet-50/50"
        />
        <section className="mt-12">
          <h2 className="font-spin-display mb-3 text-2xl font-bold text-slate-900 sm:text-3xl">
            Related Fortune Wheels
          </h2>
          <p className="mb-6 text-slate-600">
            Try another setup or return to{" "}
            <Link href={FORTUNE_WHEEL_PATH} className="font-semibold text-violet-800 underline">
              Wheel of Fortune
            </Link>
            .
          </p>
