...
import {
  applyPrizeWheelUseCase,
  getPrizeWheelUseCase,
  prizeWheelUseCaseFromTemplate,
...
function PrizeWheelAppInner({
  ...
  const [activeUseCaseId, setActiveUseCaseId] = useState<PrizeWheelUseCaseId | null>(
  ...
  const viewMode = useWheelManagerStore((state) => {
    const wheel =
      state.wheelsByTool["prize-wheel"]?.find((item) => item.id === state.currentWheelId) ||
      state.wheelsByTool["prize-wheel"]?.[0]
  ...
  const entries = useWheelManagerStore((state) => {
    const wheel =
      state.wheelsByTool["prize-wheel"]?.find((item) => item.id === state.currentWheelId) ||
      state.wheelsByTool["prize-wheel"]?.[0]
    const list = (wheel?.data as PrizeWheelData | undefined)?.entries
  ...
  const recentResults = useWheelManagerStore((state) => {
    const wheel =
      state.wheelsByTool["prize-wheel"]?.find((item) => item.id === state.currentWheelId) ||
      state.wheelsByTool["prize-wheel"]?.[0]
    const list = (wheel?.data as PrizeWheelData | undefined)?.recentResults
  ...
  const syncActionMode = useCallback(
    (mode: "normal" | "elimination" | "manual") => {
      ...
      if (wheel) {
        updateWheelData("prize-wheel", wheel.id, {
          ...(wheel.data as PrizeWheelData),
          actionMode: mode,
        })
      }
      if (mode !== "manual") {
  ...
  const resetWheelForGame = useCallback(() => {
    ...
    updateWheelData("prize-wheel", current.id, {
      ...(current.data as PrizeWheelData),
      totalSpins: 0,
      spinHistory: [],
      recentResults: [],
      selectedResult: null,
  ...
  const handleSpinCompleted = useCallback(() => {
    const current = getCurrentWheel()
    if (!current) return
    const wheelData = current.data as PrizeWheelData
    ...
    window.setTimeout(() => {
      ...
      const latest = latestWheel.data as PrizeWheelData
      ...
      if (newlyCompleted.length > 0 || newlyUnlockedThemes.length > 0) {
        updateWheelData("prize-wheel", latestWheel.id, {
          ...latest,
          ...
          {!isFullscreen && (
            <>
              <div className="mb-4 text-center">
                <ToolPageTitle
                  title={shortTitle ?? PRIZE_WHEEL_SHORT_TITLE}
                  toolType="prize-wheel"
                />
                <p className="text-gray-600">
                  {toolSubtitle ?? "Create an equal-odds prize experience"}
                </p>
              </div>
              <PrizeWheelPopularTemplates />
              {activeUseCaseId && getPrizeWheelUseCase(activeUseCaseId) && (
                <div className="mb-4 flex justify-center sm:justify-start">
                  <span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-semibold text-amber-800">
                    Template: {getPrizeWheelUseCase(activeUseCaseId)!.label}
                  </span>
                </div>
              )}
            </>
              ...
              <div className="min-w-0 lg:col-span-3">
                ...
                {!entries.length && (
                  <p className="mt-3 text-center text-sm text-slate-500">
                    Add prizes to build your wheel.
                  </p>
                )}
              </div>
            )}
