...
import { StateWheelPopularTemplates } from "@/components/state-wheel/state-wheel-popular-templates"
import type { StateWheelDeepLink } from "@/lib/state-wheel-spokes"
import { STATE_WHEEL_SHORT_TITLE } from "@/lib/state-wheel-seo"
import {
  applyStateWheelUseCase,
  stateWheelUseCaseFromTemplate,
...
const EMPTY_SELECTED_STATES: any[] = []
const EMPTY_RECENT_RESULTS: any[] = []

type StateWheelAppProps = {
  seoIntro?: ReactNode
...
function StateWheelAppInner({
  ...
  const [activeUseCaseId, setActiveUseCaseId] = useState<StateWheelUseCaseId | null>(
    deepLink?.useCaseId ?? null,
  )
  const deepLinkAppliedRef = useRef(false)
  const lastAppliedUseCaseRef = useRef<StateWheelUseCaseId | null>(null)
  ...
  const selectedStates = useWheelManagerStore((state) => {
    const wheels = state.wheelsByTool[state.currentTool]
    const wheel = wheels?.find((w) => w.id === state.currentWheelId)
    const countries = (wheel?.data as any)?.selectedStates as any[] | undefined
    return countries ?? EMPTY_SELECTED_STATES
  })
  ...
  const applyUseCasePreset = useCallback((id: StateWheelUseCaseId) => {
    const useCase = getStateWheelUseCase(id)
    if (!useCase || !applyStateWheelUseCase(id)) return
  ...
  const onSearchParams = useCallback(
    (params: URLSearchParams) => {
      ...
      const id = stateWheelUseCaseFromTemplate(params.get("template"))
      if (!id) return
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        if (!wheel) {
          createNewWheel("state-wheel", "My State Picker Wheel")
        ...
        } else if (wheel?.toolType === "state-wheel") {
          const wheelData = wheel.data as any
          if (!wheelData.selectedStates?.length) {
            const countryCode = wheelData.selectedCountry || "US"
            updateWheelData("state-wheel", wheel.id, {
              ...wheelData,
      ...
      if (wheelData.selectedResult && wheelData.selectedResult !== lastSpinResult) {
        ...
        if (result && result.name) {
          ...
          const newSpinRecord: SpinRecord = {
            ...
            timestamp: new Date(),
            result: result.name,
            options: selectedStates.map((state: any) => state.name),
          ...
          const stats = {
            ...
            fastestMemoryTime: 0,
            consecutiveDays: 1,
            totalOptions: selectedStates.length,
            aiGeneratedOptions: activeTab === "ai" ? selectedStates.length : 0,
            mysterySpins: 0,
          }
        ...
        >
          {!isFullscreen && (
            <div className="mb-4 text-center">
              <ToolPageTitle
                title={shortTitle ?? STATE_WHEEL_SHORT_TITLE}
                toolType="state-wheel"
              />
              <p className="text-gray-600">
                {toolSubtitle ?? "Pick a random state by wheel"}
              </p>
            </div>
          )}

          {!isFullscreen && <StateWheelPopularTemplates />}

          {!isFullscreen && activeUseCaseId && getStateWheelUseCase(activeUseCaseId) && (
            <div className="mb-4 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              <span className="rounded-full bg-blue-100 px-3 py-1 text-sm font-semibold text-blue-800">
                Template: {getStateWheelUseCase(activeUseCaseId)!.label}
              </span>
            </div>
          )}
                ...
                >
                  {!isFullscreen && (
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        window.dispatchEvent(new Event("open-state-results"))
                      }
...
export default function StateWheelApp(props: StateWheelAppProps) {
  return <StateWheelAppInner {...props} />
}
