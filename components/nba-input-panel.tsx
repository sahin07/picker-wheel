...
import { nbaTeams, getNBATeamsByLeague, type NBATeam, type ActionMode } from "@/data/nba-teams"
import { useToast } from "@/contexts/toast-context"
import NBATeamDetailsModal from "./nba-team-details-modal"
import NBAFavoritesModal from "./nba-favorites-modal"
import NBAComparisonModal from "./nba-comparison-modal"
...
const NBAInputPanel = React.memo(({ 
  ...
  const handleConferenceChange = (league: "all" | "Eastern" | "Western") => {
    const teams = getNBATeamsByLeague(league)
    updateWheelData("nba-wheel", wheel?.id || "", {
      ...data,
      selectedConference: league,
      selectedTeams: teams
    })
  }
  ...
  const selectAllTeams = () => {
    const teams = getNBATeamsByLeague(data.selectedConference)
    updateWheelData("nba-wheel", wheel?.id || "", {
      ...data,
      selectedTeams: teams
    })
  ...
  const applyBulkText = () => {
    ...
    const matched: NBATeam[] = []
    const missing: string[] = []
    names.forEach((name) => {
      const found = nbaTeams.find(
        (t) =>
  ...
  const addToFavorites = (team: NBATeam) => {
  ...
  const addToComparison = (team: NBATeam) => {
  ...
  const getLeagueColor = (league: string) => {
    return league === "Eastern" ? "bg-red-100 text-red-800 border-red-200" : "bg-blue-100 text-blue-800 border-blue-200"
  }

  const getDivisionColor = (division: string) => {
    const colors = {
      "Atlantic": "bg-green-100 text-green-800 border-green-200",
      "Central": "bg-purple-100 text-purple-800 border-purple-200",
      "Southeast": "bg-orange-100 text-orange-800 border-orange-200",
      "Northwest": "bg-teal-100 text-teal-800 border-teal-200",
      "Pacific": "bg-indigo-100 text-indigo-800 border-indigo-200",
      "Southwest": "bg-pink-100 text-pink-800 border-pink-200"
    }
  ...
  const handleAddManualTeam = () => {
    ...
    const allTeams = getNBATeamsByLeague(data.selectedConference)
  ...
  const handleEliminationMode = (selectedTeam: NBATeam) => {
...
Return only the team names in a simple list format.`
      ...
      const response = await fetch('/api/ai', {
        ...
        body: JSON.stringify({
          ...
          context: {
            currentSkins: [],
            userPreferences: { sport: 'basketball', league: data.selectedConference },
            chatHistory: [],
            mode: 'generator'
          }
        })
  ...
  const generateAIInsights = (team: NBATeam) => {
    const insights = {
      ...
      aiMatchup: `${team.name} would be a great rival pick against the ${['Boston Celtics', 'Los Angeles Lakers', 'Golden State Warriors', 'Chicago Bulls'][Math.floor(Math.random() * 4)]} based on historical performance`,
      ...
      historicalContext: `${team.name} last won an NBA title in ${Math.floor(Math.random() * 30) + 1980}`
    }
  ...
  const generateAICommentary = () => {
    const commentaries = [
      ...
      "The basketball gods are deciding your fate...",
      ...
      "The algorithms are working overtime like a closer in overtime!"
    ]
  ...
  const generateMoodBasedRecommendations = async (mood: string) => {
    ...
    try {
      ...
      const moodDescriptions = {
        ...
        "modern": "teams founded after 1960 representing the modern era of basketball"
      }
...
...`
  ...
  // AI Trivia
  const generateAITrivia = (team: NBATeam) => {
    const triviaQuestions = [
      {
        question: `In what year was ${team.name} founded?`,
        answer: team.founded.toString(),
        hint: "Think about the era of NBA expansion"
      },
      {
        question: `How many NBA championships has ${team.name} won?`,
      ...
      },
      {
        ...
        answer: team.manager,
        hint: "The person calling the shots from the sideline"
      }
    ]
  ...
  // AI Predictions
  const generateAIPredictions = (team: NBATeam) => {
    const predictions = {
      ...
      tradePrediction: `${team.name} might trade for a ${['point guard', 'wing', 'big', 'shooting guard'][Math.floor(Math.random() * 4)]} this season`,
      playerBreakout: `${team.name} has a rookie who could be the next big star`,
      championshipOdds: `${Math.floor(Math.random() * 15) + 5}% chance of winning the NBA championship`
    }
    setAiPrediction(predictions)
  }

  // AI Entertainment
  const generateAIEntertainment = (team: NBATeam) => {
    const entertainment = {
      teamRoast: `${team.name} is so slow, they make a turtle look like Usain Bolt! (But we still love them!)`,
      victoryDance: `When ${team.name} wins, fans do the "${team.name} Shuffle" - it's like the Macarena but with more basketball!`,
  ...
  // AI Learning
  const generateAILearning = (team: NBATeam) => {
    const learning = {
      basketballIQ: `${team.name} is known for their ${['transition offense', 'elite defense', 'three-point shooting', 'paint dominance'][Math.floor(Math.random() * 4)]}`,
  ...
  // AI Social Features
  const generateAISocial = (team: NBATeam) => {
    const social = {
      aiMatchmaker: `${Math.floor(Math.random() * 50) + 30}% of users who picked ${team.name} also liked the ${['Boston Celtics', 'Los Angeles Lakers', 'Golden State Warriors', 'Miami Heat'][Math.floor(Math.random() * 4)]}`,
...
- Head Coach: ${randomTeam.manager}
- Conference: ${randomTeam.league}
...
**Los Angeles Lakers:** A classic team with a rich history and passionate fanbase, offering a contrast to newer franchises.
...
As the sun sets over the hardwood and the crowd rises to their feet, the ${randomTeam.name} continue to write their story, one possession at a time, one game at a time, one season at a time. This is more than basketball - this is magic.
...
🎭 **Laughing? Now spin the wheel for more comedy gold!**`
  ...
  const extractSuggestedTeams = (text: string): string[] => {
    ...
    const teamNames = [
      'Los Angeles Lakers', 'Boston Celtics', 'Golden State Warriors', 'Chicago Bulls', 'Miami Heat',
      ...
      'Boston Celtics', 'Los Angeles Lakers', 'Golden State Warriors', 'Chicago Bulls'
    ]
...
Format your response as a structured analysis with specific percentages and scores.`
  ...
  const generateAIWeatherImpact = async () => {
    ...
    try {
      const prompt = `Analyze how weather conditions impact the ${randomTeam.name} NBA team's performance.
...
Format your response as a structured analysis with specific percentages and fan culture details.`
      ...
      if (response.ok) {
        ...
        if (result.success && result.data.message) {
          ...
          const fanAnalysis = {
            ...
            gameDayExperience: extractValue(aiResponse, 'game day') || `${randomTeam.homeVenue} offers one of the best fan experiences in basketball`,
      ...
      const fanAnalysis = {
        ...
        gameDayExperience: `${randomTeam.homeVenue} offers one of the best fan experiences in basketball`,
  ...
  const generateAllAIFeatures = (team: NBATeam) => {
    setIsGeneratingAI(true)
  ...
  const generateAIPredictionFromInput = async (userDescription: string) => {
    ...
    try {
      const prompt = `Analyze this basketball scenario and provide a detailed prediction:

User Description: "${userDescription}"

Available NBA Teams:
${nbaTeams.map(team => `- ${team.name} (${team.city}, ${team.league} Conference, ${team.division} Division, Founded: ${team.founded}, Championships: ${team.championships}, Home: ${team.homeVenue}, Head Coach: ${team.manager})`).join('\n')}
...
Make your prediction engaging, detailed, and basketball-focused. Include specific team names and reasoning.`
      ...
      const response = await fetch('/api/ai', {
        ...
        body: JSON.stringify({
          ...
          context: {
            currentSkins: [],
            userPreferences: { sport: 'basketball', analysis: 'prediction' },
            chatHistory: [],
            mode: 'analysis'
          }
        })
      ...
      const opponent = nbaTeams.filter(t => t.id !== randomTeam.id)[Math.floor(Math.random() * (nbaTeams.length - 1))]
      ...
      const predictions = [
        ...
        `My AI analysis suggests ${randomTeam.name} will dominate this matchup. Their ${randomTeam.league} conference experience and home advantage at ${randomTeam.homeVenue} will be key factors.`,
        ...
        `Based on your input, ${randomTeam.name} has the momentum and team chemistry to secure a victory. Their recent performance trends and ${randomTeam.league} conference standing position them well for success.`
      ]
  ...
  const conferences = [
    ...
    { id: "Eastern", name: "Eastern Conference" },
    { id: "Western", name: "Western Conference" }
  ]
      ...
      <div className="flex max-h-[min(70vh,36rem)] min-h-0 w-full min-w-0 flex-col overflow-hidden rounded-lg border bg-white shadow-sm lg:max-h-none">
        <div className="flex shrink-0 items-center justify-between gap-1 border-b bg-slate-50/80 px-2 py-2 sm:gap-2 sm:px-3">
          <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
            <p className="truncate text-xs font-semibold text-slate-800 sm:text-sm">NBA Controls</p>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4">
                {conferences.map((conference) => (
                  <Button
                    ...
                    onClick={() => handleConferenceChange(conference.id as "all" | "Eastern" | "Western")}
                  >
                    <div className={`flex h-8 w-8 items-center justify-center rounded-full shadow-md sm:h-10 sm:w-10 ${
                      data.selectedConference === conference.id 
                        ? "bg-white/20 backdrop-blur-sm" 
                        : getLeagueBgColor(conference.id)
                    }`}>
                      <span className="text-sm text-white sm:text-lg">
                        {getLeagueIcon(conference.id)}
                      </span>
                    </div>
                    <span className={`line-clamp-2 px-0.5 text-center text-[10px] font-semibold leading-tight sm:text-xs ${data.selectedConference === conference.id ? "text-white" : "text-blue-700"}`}>
                      {conference.name}
                    </span>
                  </Button>
                ))}
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-gradient-to-br from-violet-50 to-purple-50 border-violet-200">
              <CardHeader>
                ...
                <p className="text-violet-700 text-sm">Unleash the power of AI to create unique basketball experiences</p>
              </CardHeader>
            ...
            <Card className="bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-200">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Textarea
                    placeholder="e.g., 'Celtics vs Lakers, Celtics won 3 games in a row, home court advantage, strong defense, recent injuries...'"
                    value={userInput}
                  ...
                  <div className="space-y-3">
                    <div className="p-3 bg-emerald-50 rounded-lg">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-emerald-800">Basketball IQ</span>
                      </div>
                      <p className="text-sm text-emerald-700">{aiLearning.basketballIQ}</p>
                    </div>
            ...
            <Card className="bg-gradient-to-br from-indigo-50 to-blue-50 border-indigo-200">
              <CardHeader>
                ...
                <p className="text-indigo-700 text-sm">Deep dive into advanced basketball analytics and insights</p>
              </CardHeader>
              <CardContent className="space-y-4">
                {/* Analysis Tools Grid */}
            ...
            <Card className="bg-gradient-to-br from-purple-50 to-pink-50 border-purple-200">
              <CardContent className="p-6">
                <div className="text-center">
                  ...
                  <p className="text-xs text-purple-600 mt-2 font-medium">Experience the full AI-powered NBA experience!</p>
                </div>
              </CardContent>
            </Card>
          </div>
