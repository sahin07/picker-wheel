...
import type { ActionMode, DisplayMode, Pokemon, SpinResult } from "@/types/pokemon-types"
import type { WheelTheme } from "@/lib/picker-wheel-themes"
import { useSettingsStore } from "@/stores/settings-store"

export interface PokemonWheelSectionProps {
  pokemon: Pokemon[]
  rotation: number
  isSpinning: boolean
  spinDuration: number
  ...
  manualPokemonName: string
  onManualPokemonNameChange: (value: string) => void
  onAddManualPokemon: () => void
...
export default function PokemonWheelSection({
  pokemon,
  rotation,
  isSpinning,
  spinDuration,
  ...
  return (
    ...
    >
      <div className="relative mx-auto flex w-full max-w-[680px] flex-col items-center">
        <div
          className={`relative w-full max-w-[680px] overflow-visible ${
            !isSpinning && pokemon.length > 0 ? "cursor-pointer" : ""
          }`}
          onClick={!isSpinning && pokemon.length > 0 ? onSpin : undefined}
        >
          <WheelComponent
            key={wheelKey}
            pokemon={pokemon}
      ...
      <div className="flex justify-center">
        <div className="rounded-lg bg-gradient-to-r from-amber-500 to-red-500 px-4 py-2 text-white shadow-lg">
      ...
      <Button
        type="button"
        onClick={onSpin}
        disabled={isSpinning || pokemon.length === 0}
        className="w-full max-w-sm bg-gradient-to-r from-red-500 to-amber-500 px-6 py-3 text-base font-semibold text-white shadow-lg transition-all duration-200 hover:from-red-600 hover:to-amber-600 hover:shadow-xl sm:px-12 sm:text-lg"
      >
        {isSpinning ? (
          ...
          `⚡ SPIN THE WHEEL (${pokemon.length} Pokémon)`
        )}
      </Button>

      {pokemon.length === 0 && (
        <p className="mt-2 text-center text-gray-500">Select some Pokémon to start spinning!</p>
      )}
      ...
      {actionMode === "manual" && (
        <div className="flex min-w-0 items-center gap-2">
          <Input
            ...
            onChange={(e) => onManualPokemonNameChange(e.target.value)}
            placeholder="Type Pokémon name..."
            className="min-w-0 w-40 text-sm sm:w-48"
            onKeyDown={(e) => {
              if (e.key === "Enter" && manualPokemonName.trim()) onAddManualPokemon()
            }}
          />
          <Button
            onClick={onAddManualPokemon}
      ...
      <div className="mt-2 w-full max-w-md rounded-xl border border-red-200 bg-gradient-to-br from-amber-50 to-red-50 p-3 sm:mt-4 sm:p-4">
            ...
            <label
              ...
              className={`flex cursor-pointer flex-col items-center space-y-1 rounded-lg p-2 transition-all duration-200 sm:space-y-2 sm:p-3 ${
                actionMode === mode.value
                  ? "bg-gradient-to-br from-red-500 to-amber-500 text-white shadow-lg shadow-red-500/25"
                  : "border border-red-200 bg-white hover:border-red-300 hover:bg-gradient-to-br hover:from-amber-50 hover:to-red-50"
              }`}
            >
              <input
                type="radio"
                name="pokemonActionMode"
                value={mode.value}
        ...
        <div className="mt-2 rounded-lg bg-red-50 p-2 text-center text-[11px] text-red-600 sm:mt-3 sm:text-xs">
          {actionMode === "normal" && "🎯 All Pokémon available for each spin"}
          {actionMode === "elimination" && "❌ Selected Pokémon is removed after each spin"}
          {actionMode === "manual" && "📝 Add custom Pokémon by typing names"}
        </div>
      </div>

      {currentResult && !isSpinning && (
        <div className="w-full max-w-md rounded-xl border-2 border-green-300 bg-gradient-to-r from-green-100 to-amber-100 p-4 text-center shadow-lg sm:p-6">
          ...
          <div className="mb-1 flex items-center justify-center gap-2 sm:gap-3">
            <span className="text-3xl sm:text-4xl">{currentResult.pokemon.emoji}</span>
            <div className="min-w-0 text-left">
              <p className="truncate text-xl font-bold text-gray-900 sm:text-2xl">
                {currentResult.pokemon.name}
              </p>
              <p className="text-xs text-gray-600 sm:text-sm">
                {currentResult.pokemon.type.join(" ")}
              </p>
            </div>
          </div>
        </div>
